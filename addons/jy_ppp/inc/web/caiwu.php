...
global $_W,$_GPC;
		...
		if($op=='credit')
		{
			$credit_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.log=1 ".$condition2."LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
		...
		if($op=='baoyue')
		{
			$baoyue_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.log=2 ".$condition2."LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
		...
		if($op=='shouxin')
		{
			$shouxin_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.log=3 ".$condition2."LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
