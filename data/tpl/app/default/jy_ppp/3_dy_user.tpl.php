...
<div class="bodybox">
	...
	<!-- begin -->
	<?php  $now=time();?>
	<?php  if(is_array($xuni)) { foreach($xuni as $x) { ?>
		<div class="conDiv">
			<div class="ovfl conLeft">
				<div class="floL leftDiv">
					<?php  if(!empty($x['avatar'])) { ?>
                    ...
                    <?php  } else { ?>
                        ...
                        <?php  } ?>
                    <?php  } ?>
					<div class="floR nowrap mainColor"><?php  echo $x['nicheng'];?></div>
					<div class="floR nowrap mainColor"><?php  if(!empty($x['brith'])) { ?>
                <?php  $birthday=$x['brith'];$month=0;if(date('m', $now)>date('m', $birthday))$month=1;if(date('m', $now)==date('m', $birthday))if(date('d', $now)>=date('d', $birthday))$month=1;$nianlin=date('Y', $now)-date('Y', $birthday)+$month;?><?php  echo $nianlin;?>岁<?php  } ?> <?php  echo $province[$x['province']];?></div>
				</div>
				<div class="floR rightDiv txtAC"><a href="<?php  echo $this->createMobileUrl('geren',array('xuniid'=>$x['id']))?>">编辑</a></div>
			</div>
			<div class="ovfl tipDiv">
				<a href="<?php  echo $this->createMobileUrl('dy_msg',array('userid'=>$x['id']))?>" class="floL txtAC">未读消息:<?php  if(!empty($x['weidunum'])) { ?><?php  echo $x['weidunum'];?><?php  } else { ?>0<?php  } ?></a>
				<a href="<?php  echo $this->createMobileUrl('dy_man',array('userid'=>$x['id']))?>" class="floL txtAC">关注用户:<?php  if(!empty($x['attentnum'])) { ?><?php  echo $x['attentnum'];?><?php  } else { ?>0<?php  } ?></a>
			</div>
